            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? "border-accent bg-accent/10" : "border-border bg-surface-muted/40 hover:border-border-strong"
                )}
              >
        ...
        return (
          <Card key={outcome.scenario.id}>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {diffs.map((diff) => (
                <div key={diff.label} className="rounded-control border border-border bg-surface-muted/40 px-3 py-2">
                  <p className="text-[11px] font-semibold text-muted">{diff.label}</p>
            ...
            {outcome.issues.filter((issue) => issue.type === "error").length > 0 && (
              <div className="mt-3 space-y-1">
                <p className="text-xs font-semibold text-danger">Vincoli non soddisfatti in questo scenario</p>
...
function Delta({ delta }: { delta: number | null }) {
  if (delta === null || delta === 0) {
    return <span className="flex items-center gap-1 text-[11px] text-muted"><Minus className="size-3" />nessuna variazione</span>;
  }
  const positive = delta > 0;
  return (
    <span className={cn("flex items-center gap-1 text-[11px]", positive ? "text-warning" : "text-success")}>
